<<<<<<< HEAD:src-js/services/razorpay.js
...
const KEY_ID = import.meta.env.VITE_RAZORPAY_KEY_ID;
    ...
    rzp.open();
=======
import { loadScript, RAZORPAY_SDK } from '../utils/loadScript.js';
import { createRazorpayOrder, verifyRazorpayPayment } from './api.js';

export const initiateRazorpayPayment = async ({ amount, customer, items, onSuccess, onFailure }) => {
  const KEY_ID = import.meta.env.VITE_RAZORPAY_KEY_ID;

  // If no Razorpay key is configured, simulate payment for demo
  if (!KEY_ID) {
    console.warn('No VITE_RAZORPAY_KEY_ID set — simulating payment for demo');
    await new Promise(r => setTimeout(r, 1500));
    onSuccess({ razorpay_payment_id: 'pay_demo_' + Date.now(), razorpay_order_id: 'order_demo_' + Date.now(), razorpay_signature: 'demo_sig' });
  ...
  rzp.open();
>>>>>>> 1eed3bb41956043c2cd813d08cf28aeeaa2efc64:src/services/razorpay.js
};
